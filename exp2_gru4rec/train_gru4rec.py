"""
Experiment 2: GRU4Rec with Linear Attention (SLIDE backbone)
Replaces the LSTM with a 2-layer GRU + linear attention module.
Sessions still fragmented at file boundaries (same loading as Exp 1).
Result: HR@10 ≈ 5%
"""
...
class LinearAttention(nn.Module):
    """Single learned projection + softmax aggregation (SLIDE paper)."""
...
class GRU4RecLinearAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        context  = self.attn(out)                    # (B, H)
        return self.fc(self.dropout(context))         # (B, vocab)

model = GRU4RecLinearAttn(svd_matrix, HIDDEN_DIM, NUM_LAYERS, DROPOUT).to(device)
...
model.load_state_dict(torch.load("best_gru4rec.pt"))
print(f"\nTest HR@10 = {evaluate(model, test_loader):.2f}%")
