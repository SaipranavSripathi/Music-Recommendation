"""
AU2ACTR - Full Dataset Training
Uses all available users and sessions from the Deezer dataset

Run with:
    cd /Users/spartan/Desktop/CS274/Music-Recommendation
    python3 train_full_dataset.py

The script will:
1. Load ALL user sessions from the dataset
2. Train the AU2ACTR model
...
# Model parameters
MIN_SESSIONS = 5        # Minimum sessions per user (lower = more users)
...
print("="*60)
print("AU2ACTR - Full Dataset Training")
print("="*60)
print(f"Config: MIN_SESSIONS={MIN_SESSIONS}, SEQ_LEN={SEQ_LEN}")
...
session_cache = CACHE_DIR / f"all_sessions_s{MIN_SESSIONS}.pkl"
...
if session_cache.exists():
    ...
    print("Loading ALL user sessions from dataset...")
    ...
    eligible_users = {u for u, c in user_counts.items() if c >= MIN_SESSIONS}
    ...
    # Sort sessions by first track ID (proxy for timestamp)
    for u in sessions:
...
class AU2ACTR(nn.Module):
    def __init__(self, n_tracks, dim=128, hidden=256):
        ...
        self.pos = PositionalEncoding(dim, m=100)
        
        enc = nn.TransformerEncoderLayer(
            d_model=dim, nhead=NUM_HEADS, dim_feedforward=hidden,
            dropout=0, activation='gelu', batch_first=True, norm_first=True
        )
        self.trans = nn.TransformerEncoder(enc, num_layers=NUM_BLOCKS, enable_nested_tensor=False)
...
class SessionDataset(Dataset):
    def __init__(self, sess, t2id, seq=10, min_s=5):
        ...
        for u, s in sess.items():
            ...
            train = s[:-3]  # Reserve last 3 for val/test
...
for epoch in range(EPOCHS):
    ...
    if acc > best_acc:
        best_acc = acc
        checkpoint_path = CACHE_DIR / "best_full_dataset.pt"
...
print(f"Checkpoint saved to: {CACHE_DIR / 'best_full_dataset.pt'}")
